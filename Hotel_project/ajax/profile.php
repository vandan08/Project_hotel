...
if (isset($_POST['profile_form'])) {
    ...
    //fetching old image and deleting old image 
    $u_exist = select('SELECT * FROM `user_cred` WHERE `id`=? LIMIT 1',
    [$_SESSION['uId']],'s');

    $u_fetch = mysqli_fetch_assoc($u_exist);

    delete_image($u_fetch['profile'],USER_FOLDER);
...
if (isset($_POST['pass_form'])) {
    ...
    if ($frm_data['new_pass']!=$frm_data['confim_pass']) {
        echo 'mismatch';
        exit;
    }


    $enc_pass = password_hash($frm_data['new_pass'],PASSWORD_BCRYPT);  //here we are encypting password with this 'PASSWORD_BCRYPT' algorithm which is inbuilt algorithm 
